...
// against user input using cosine similarity.
...
// NewVectorMatcher builds a vector matcher from a list of lazy skills.
// Call this instead of BuildTriggerIndex to get semantic matching.
func NewVectorMatcher(skills []Skill, cfg MatcherConfig) *VectorMatcher {
	...
	vm := &VectorMatcher{
		...
		rp:     vector.NewRandomProjections(cfg.OutputDim),
		skills: make([]Skill, 0, len(skills)),
		ids:    make([]string, 0, len(skills)),
		cfg:    cfg,
	}

	// Build corpus texts for RP vocabulary
	corpus := make([]string, 0, len(skills))
	for _, s := range skills {
		text := buildEmbedText(s, cfg)
		corpus = append(corpus, text)
	}

	// Fit the RP vocabulary from all skill texts
	vm.rp.Fit(corpus)

	// Embed each skill and add to store
	for i, s := range skills {
		vec, err := vm.rp.Embed(corpus[i])
		if err != nil || vec == nil {
			continue // skip skills that fail to embed
		}
		vm.store.Add(s.Name, vec)
...
func (vm *VectorMatcher) MatchSkills(userInput string, maxSlots int) []Skill {
	...
	queryVec, err := vm.rp.Embed(userInput)
